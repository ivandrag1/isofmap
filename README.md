...
### 2) Стартиране с адрес
```bash
python main.py --address 'гр. София, район Лозенец, ж.к. "Лозенец", ул. "Милин камък" №50-54'
```

Ще получиш JSON директно в терминала.

### 3) Запис в JSON файл
```bash
python main.py \
  --address 'София, ул. Милин камък 50-54' \
  --output sample_output/milin_kamak.json
```

### 4) Какво да очакваш в резултата
- `match_status`: `exact`, `partial`, `not_found`, `ambiguous`
- `target_property`: данни за намерения имот (и всички сурови атрибути в `attributes_raw`)
- `neighboring_properties`: съседни имоти, ако могат да се извлекат надеждно
- `errors` / `uncertainties`: ако липсва информация или има нееднозначност
